...
// only work when the element sits at its original position in the DOM â€”
...
const testimonials = [
  ...
  { name: "Esther Williams", initials: "EW", role: "Research & Development Professional", avatar: "avatar-gold", quote: "Zorbit's approach to data was refreshing. They didn't just create chartsâ€”they helped organise the information in a way that made the findings easier to communicate and understand. The dashboard was clear, professional, and useful for presenting insights to stakeholders." },
  { name: "Ibrahim Musa", initials: "IM", role: "Store Owner Â· Phones & Accessories Â· Kaduna State", avatar: "avatar-lime", quote: "The inventory software developed for us helped bring more structure to how we track our products and records. The team listened to our requirements and made adjustments where necessary. It has made day-to-day management much easier.", caseStudy: { href: "/case-studies/inventory-software", label: "Read inventory case study" } },
...
export default function Home() {
  ...
  // startLogin() during render (no href={startLogin()}) â€” it mints a one-time
  ...
  const handleInquirySubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // separate backend â€” this route ships in the same Vercel deploy.
      ...
      window.location.href = `mailto:hello@zorbittechnology.com.ng.ng?subject=${encodeURIComponent("New Zorbit project inquiry")}&body=${encodeURIComponent(detail)}`;
  ...
  return <div className="zorbit-page">
    ...
    <main>
      ...
      <section id="about" className="about-section section-shell"><div className="about-statement"><span className="section-index">04 / ABOUT ZORBIT</span><h2>Useful technology<br /><span>starts with access.</span></h2><div className="about-profile"><div className="profile-role">COMPANY / OPERATING PRINCIPLE</div><h3>Clarity is<br />the product.</h3><p className="profile-specialism">Data Â· Digital products Â· Useful systems</p></div><p>Zorbit is a technology company built to make data and digital tools more accessible to SMEs, businesses, and individuals across all states. We combine data, design, and delivery to help clients move from scattered inputs to practical next stepsâ€”without pricing useful technology out of reach.</p><p>Our mission is to offer clear, reliable services at fair and affordable prices. Our work begins with the operating point: the decision to improve, the process to simplify, the audience to reach, or the opportunity to make usable. From there, we build with evidence, clarity, and a focus on what will keep working after launch.</p><div className="about-signals"><span>AFFORDABLE ACCESS</span><span>SMES &amp; BUSINESSES</span><span>NATIONWIDE DELIVERY</span></div><div className="about-links"><a className="text-link" href="/about">About Zorbit <ArrowUpRight size={16} /></a><a className="text-link text-link-muted" href="/founder">Meet the founder <ArrowUpRight size={16} /></a></div></div><div className="credential-card"><div className="credential-head"><ShieldCheck size={20} /><span>COMPANY / PROFILE</span></div><div className="credential-big">Built for<br /><strong>access.</strong></div><dl><div><dt>Status</dt><dd>CAC Registered Business</dd></div><div><dt>Founded</dt><dd>2024</dd></div><div><dt>Focus</dt><dd>Data Â· Digital Â· Intelligence</dd></div><div><dt>Reach</dt><dd>All States + Remote</dd></div></dl><div className="credential-seal"><Check size={15} /> VERIFIED OPERATOR</div></div></section>

      <section id="work" className="content-section portfolio-section section-shell"><div className="section-heading"><div><span className="section-index">05 / SELECTED WORK</span><h2>Evidence over<br /><span>empty promises.</span></h2></div><p>Every project is framed as proof of a clearer decision, cleaner operation, or more usable systemâ€”across academic services, company websites, business software, and digital commerce.</p></div><div className="portfolio-feature"><div id="website-case-studies" className="portfolio-art">{featuredWebsites.map((website, i) => <PortfolioWebsiteThumb website={website} index={i} key={website.label} />)}</div><div className="project-list">{projects.map((project, i) => <ProjectCard project={project} index={i} key={project.title} />)}</div></div></section>
