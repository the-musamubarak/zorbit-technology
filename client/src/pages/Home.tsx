...
const projects = [
  ...
  { number: "03", title: "Inventory & Debt Tracking Software", tools: ["Inventory control", "Debt tracking", "Business reporting"], text: "A single operating view that helps a client see stock position, sales movement, outstanding debt, and daily records before they become leaks." },
  { number: "04", title: "NITDA Digital File Tracking Dashboard", tools: ["Power BI", "DAX", "Python"], text: "A usable audit trail that helps teams see file movement, bottlenecks, and operational throughput with less guesswork." },
...
export default function Home() {
  ...
  return <div className="zorbit-page">
    ...
    <main>
      ...
      <section className="dashboard-showcase section-shell" aria-labelledby="dashboard-showcase-title"><div className="dashboard-heading"><div><span className="section-index">03 / DATA EVIDENCE</span><h2 id="dashboard-showcase-title">Dashboards that<br /><span>make the signal useful.</span></h2></div><p>Selected Power BI work showing how dense information can become an operating view for teams, institutions, and businesses.</p></div><div className="dashboard-grid">{dashboardShowcase.map((dashboard) => <article className="dashboard-card" key={dashboard.title}><a className="dashboard-image" href={dashboard.href} target="_blank" rel="noreferrer" aria-label={`Open ${dashboard.title} in Power BI`}><img src={dashboard.image} alt={`${dashboard.title} preview`} loading="lazy" /><span>OPEN DASHBOARD <ArrowUpRight size={15} /></span><div className="dashboard-tools" aria-label={`Tools used: ${dashboard.tools.join(", ")}`}><small>TOOLS USED</small><div>{dashboard.tools.map((tool) => <b key={tool}>{tool}</b>)}</div></div></a><div className="dashboard-copy"><span>{dashboard.number} / POWER BI</span><h3>{dashboard.title}</h3><p>{dashboard.caption}</p><a href={dashboard.href} target="_blank" rel="noreferrer">View dashboard <ArrowUpRight size={14} /></a></div></article>)}</div></section>

      <section id="about" className="about-section section-shell"><div className="about-statement"><span className="section-index">04 / ABOUT ZORBIT</span><h2>Useful technology<br /><span>starts with listening.</span></h2><div className="about-profile"><div className="profile-role">FOUNDER / TECHNICAL LEAD</div><h3>Musa Mubarak<br />Abdulaziz.</h3><p className="profile-specialism">Data Analyst · Power BI Developer · Full-Stack Developer</p></div><p>Zorbit is led by Musa Mubarak Abdulaziz, a multidisciplinary technology professional working at the intersection of data, product, and delivery. He helps companies, institutions, and individuals turn scattered information into useful systems—whether that means a clear dashboard, a credible website, a focused web application, or operational software that keeps the work moving.</p><p>His approach combines analytical thinking with practical build quality. The goal is never technology for display; it is visibility for better decisions, digital experiences that earn trust, and tools people can use with confidence.</p><div className="about-signals"><span>DATA &amp; INSIGHT</span><span>POWER BI &amp; VISUALIZATION</span><span>WEBSITES &amp; SOFTWARE</span></div><a className="text-link" href="#contact">Start a conversation <ArrowUpRight size={16} /></a></div><div className="credential-card"><div className="credential-head"><ShieldCheck size={20} /><span>OPERATOR / PROFILE</span></div><div className="credential-big">Built in<br /><strong>Abuja.</strong></div><dl><div><dt>Role</dt><dd>Data &amp; Technology Lead</dd></div><div><dt>Speciality</dt><dd>Power BI · Full-Stack</dd></div><div><dt>Base</dt><dd>Abuja, Nigeria</dd></div><div><dt>Reach</dt><dd>Nigeria + Remote</dd></div><div><dt>Status</dt><dd>CAC Registered Business</dd></div></dl><div className="credential-seal"><Check size={15} /> VERIFIED OPERATOR</div></div></section>

      <section id="work" className="content-section portfolio-section section-shell"><div className="section-heading"><div><span className="section-index">05 / SELECTED WORK</span><h2>Evidence over<br /><span>empty promises.</span></h2></div><p>Every project is framed as proof of a clearer decision, cleaner operation, or more usable system—across academic services, company websites, business software, and digital commerce.</p></div><div className="portfolio-feature"><div className="portfolio-art">{featuredWebsites.map((website) => <a className="portfolio-website" href={website.href} target="_blank" rel="noreferrer" key={website.label} aria-label={`Visit ${website.title}`}><img src={website.image} alt={website.title} loading="lazy" /><span className="art-label">{website.label} <ArrowUpRight size={13} /></span></a>)}</div><div className="project-list">{projects.map((project) => <article className="project" key={project.title}><span className="project-number">{project.number}</span><div><h3>{project.title}</h3><p>{project.text}</p><div className="tags">{project.tools.map(tool => <span key={tool}>{tool}</span>)}</div>{project.href && <a className="project-link" href={project.href} target="_blank" rel="noreferrer">{project.linkLabel} <ArrowUpRight size={14} /></a>}</div>{project.href ? <a href={project.href} className="project-arrow" target="_blank" rel="noreferrer" aria-label={`Visit ${project.title}`}><ArrowUpRight size={20} /></a> : <ArrowUpRight className="project-arrow" size={20} />}</article>)}</div></div></section>
